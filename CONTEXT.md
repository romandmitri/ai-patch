...
- `Patchlet` is the canonical immutable facade for schema generation and patch application.
- `ContentFormat` currently contains only `Text`.
- `OperationType` contains `Replace`, `InsertBefore`, `InsertAfter`, and `Delete`.
- `Operation`, `PatchletOptions`, and the four operation-specific types are exported for typed construction.

## Architecture

- `src/patchlet/Patchlet.ts`: validates construction, composes operation schemas, validates direct application input, and dispatches by format.
- `src/operation/`: owns the operation union, discriminants, operation-specific types, and strict Zod schemas.
- `src/patcher/Patcher.ts`: abstract internal patch-engine contract over immutable content.
- `src/patcher/Patcher_Text.ts`: normalizes text, resolves original line anchors to concrete edits, validates conflicts, and applies edits.
- `src/patch/Patch.ts`: normalized internal edit representation using source input indexes and original-content offsets.
- `src/content/`: owns semantic aliases for complete content, partial content, line numbers, offsets, and supported formats.
- `src/patchlet/test/`: covers operation application, complex batches, schema validation, and composition.
...
- `Patchlet.patch()` validates the complete operation list through the same Zod schema exposed by `toSchema()`.
...
- Unsupported content formats fail during `Patchlet` construction with `Error`.
...
- `npm test`: runs all `src/patchlet/test/*.test.ts` files with Node's test runner and `tsx`.
